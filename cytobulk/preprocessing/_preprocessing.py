...
def _join_marker(bulk_adata,sc_adata,annotation_key,marker_list,out_dir='./',dataset_name='',python_path=None,giotto_gene_num=150):
    ...
    if not exists(f'{out_dir}/{dataset_name}_marker.txt'):
        ...
        marker_gene = find_marker_giotto(sc_adata,annotation_key,out_dir,dataset_name,python_path,giotto_gene_num)
...
def find_markers_wilcoxon(adata, groupby='cell_type', n_genes=100, 
    ...
    for group in groups:
        ...
        count = 0
        for gene, pval, logfc in zip(genes, pvals, logfcs):
            if pval < pval_cutoff and abs(logfc) > logfc_cutoff:
                if gene not in marker_gene:
                    marker_gene.append(gene)
                count += 1
                if count >= n_genes:
                    break
...
def preprocessing(bulk_data,
    ...
    **kwargs: 
    ...
    new_annotation_key = 'curated_cell_type'
    print('================================================================================================================')
    print('------------------Start to check cell type annotation and quality control...------------------')
    sc_adata = sc_adata[sc_adata.obs[new_annotation_key]!="invalid",:]
    #sc_adata = sc_adata[:, ~sc_adata.var_names.str.startswith(('MT-', 'mt-'))].copy()
    #print(f"sc_adata: {sc_adata.n_vars} genes remaining")
    # Remove MT genes from bulk_data
    #bulk_data = bulk_data[:, ~bulk_data.var_names.str.startswith(('MT-', 'mt-'))].copy()
    #print(f"bulk_data: {bulk_data.n_vars} genes remaining")
    if is_st:
        sc_adata, bulk_adata, common_gene = qc_st_sc(bulk_data,sc_adata,out_dir=out_dir,save=save,dataset_name=dataset_name,sc_st=skip_find_markers,**kwargs)
    else:
        sc_adata, bulk_adata, common_gene = qc_bulk_sc(bulk_data,sc_adata,out_dir=out_dir,save=save,dataset_name=dataset_name,**kwargs)
    
    if 'curated_cell_type' not in sc_adata.obs.columns:
        sc_adata.obs.rename(columns={annotation_key: 'curated_cell_type'}, inplace=True)
    ...
    if bulk_hvg:
        ...
        print(f'The number of valid genes of bulk data is {bulk_adata.shape[1]}')
    print('------------------Finish------------------')
    ...
    '''
    if not skip_find_markers:
        if marker_method == "giotto":
            print('Start to find vaild marker genes with giotto')
            marker_gene = _join_marker(bulk_adata,sc_adata,annotation_key,marker_list,out_dir,dataset_name,giotto_gene_num=giotto_gene_num)
        elif marker_method == "wilcoxon":
            print('Start to find vaild marker genes using wilcoxon')
            marker_gene = find_markers_wilcoxon(
                sc_adata,
