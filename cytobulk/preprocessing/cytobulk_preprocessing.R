...
run_giotto <- function(sc_data, sc_anno, python_path, out_dir, project, giotto_gene_num,save=TRUE){
  ...
  scran_markers_subclusters = findMarkers_one_vs_all(gobject = sc_giotto_object,
                                                    ...
                                                    cluster_column = "leiden_clus") #leiden_clus
  id<-sc_giotto_object@cell_metadata$curated_cell_type

  Sig_scran <- unique(scran_markers_subclusters$genes[which(scran_markers_subclusters$ranking <= giotto_gene_num)])
