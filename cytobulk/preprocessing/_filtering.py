...
def high_variable_gene(adata, flavor):
    ...
    if adata.var.highly_variable.sum() > 4000:
        sc.pp.highly_variable_genes(adata, min_mean=0, max_mean=np.inf, min_disp=0.5, flavor=flavor)
    if adata.var.highly_variable.sum() < 200:
        sc.pp.highly_variable_genes(adata, min_mean=0, max_mean=np.inf, min_disp=0.15, flavor=flavor)
