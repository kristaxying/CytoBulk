...
def find_marker_giotto(sc_adata,anno_key,out_dir='./',project='',python_path=None,giotto_gene_num=150):
    """
    ...
    pandas2ri.activate()
    robjects.r.run_giotto(exp,sc_anno,python_path,out_dir,project,giotto_gene_num)
...
def remove_batch_effect(pseudo_bulk, bulk_adata, out_dir, project='', batch_effect=True):
    """
    Remove batch effect between pseudo_bulk and input bulk data using scanpy's combat.
    ...
    if batch_effect:
        ...
        pseudo_bulk_df = get.count_data(pseudo_bulk)
        input_bulk_df = get.count_data(bulk_adata)
        all_df = pd.concat([pseudo_bulk_df, input_bulk_df], axis=1)
        all_df = all_df.T  


        batch = np.array([0] * pseudo_bulk_df.shape[1] + [1] * input_bulk_df.shape[1])
        batch = pd.Categorical(batch)

        adata = sc.AnnData(all_df)
        adata.obs['batch'] = batch

        sc.pp.combat(adata, key='batch')

        adata_df = pd.DataFrame(adata.X, index=adata.obs_names, columns=adata.var_names).T 
        adata_df.to_csv(f"{out_dir}/{project}_batch_effected.txt", sep='\t')


        pseudo_bulk.layers["batch_effected"] = adata_df.T.loc[pseudo_bulk.obs_names, :].values
        bulk_adata.layers["batch_effected"] = adata_df.T.loc[bulk_adata.obs_names, :].values
