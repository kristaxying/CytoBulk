...
def get_cell_type_fraction(number_of_cells, cell_type_fraction_data):
    ...
    cell_type_numbers = cell_type_fraction_data.copy()
...
def estimate_cell_number_RNA_reads(st_data, mean_cell_numbers):
    ...
    RNA_reads = np.sum(expressions, axis=0, dtype=float)
...
def run_cytospace(st_adata,sc_adata,deconv_result,n_cells_per_spot_data,scRNA_max_transcripts_per_cell=1500,
                   annotation_key="cell_type",downsample_off=False,output_folder="cytospace_results", output_prefix="",
    ...
    # Check paths
    output_path = utils.check_paths(output_folder, output_prefix)
