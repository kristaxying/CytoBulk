...
def _bulk_mapping_parallel(i, cell_num, bulk_data, sc_data, cell_list, meta_dict, cellname_list):
    """
    ...
    mapped_cor = utils.pear(sample,bulk_data[name].values)
    mapped_cor = mapped_cor.item()
    print(f"sample {name} done.")

    return sample.tolist(), mapped_cor, sc_mapping_dict
...
def bulk_mapping(frac_data,
    ...
    if multiprocessing:
        ...
        with Pool(cpu_num) as p:
            results = p.starmap(_bulk_mapping_parallel, [(i, cell_num[i], bulk_data, sc_data, cell_list, meta_dict, cellname_list) for i in range(len(cell_num))])
        # postprocessing
        for i, (sample_i, mapped_cor_i, sc_mapping_dict_i) in enumerate(results):
