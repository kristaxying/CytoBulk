...
def _get_prop_specific_sti(sc_data,meta_data,n_celltype,cell_specific,annotation_key,n_sample,n,round_th,project,set_missing=False):
    ...
    if cell_prop.shape[1] > 5:
        for j in range(int(cell_prop.shape[0])):
          cells = np.random.choice(np.arange(cell_prop.shape[1]), replace=False, size=cell_prop.shape[1]-4).tolist()
    ...
    if cell_prop.shape[1]<=5:
        ...
        cell_prop[:,selected_index]=cell_prop[:,selected_index]+(1/2)
    cell_prop = cell_prop / np.sum(cell_prop, axis=1).reshape(-1, 1)
...
def st_simulation(sc_adata,
    ...
    if exists(f"{reference_out_dir}/filtered_{project}_st.h5ad"):
        ...
        if not average_ref:
            ...
            selected_index = np.unique(np.argsort(-similarity_matrix, axis=1)[:, :5].flatten())
            selected_cell = np.array(average_cell_exp.index.tolist())[selected_index]
